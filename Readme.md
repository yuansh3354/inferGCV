...
> 作为肿瘤生态位的核心，我们对功能程序和癌细胞性质的理解仍处于初级阶段。其中一个主要原因是癌细胞中强烈的转录异质性远高于非癌细胞，这使得研究人员无法进一步探索特定癌症的分子基础、亚型以及癌细胞的进展轨迹。识别癌症异常活动是精准肿瘤学的基本目标之一。在这项研究中，我们应用对比学习方法来分析 scRNA-seq 数据并开发了一个 sceAPA（评估异常通路活性）模型来构建癌症中 RNA 特征和基因组变异相关异常通路活性（APA）之间的空间映射相关性。细胞。 scRNA-seq 揭示的动态转录谱实时反映了癌细胞重新配置转录组模式和开发各种 APA 以响应选择压力以获得适应性进化优势。 sceAPA能够以单细胞单基因分辨率捕获癌细胞内APA的微弱信号，并分析APA多样性在不同选择压力下赋予癌症单细胞的适应性进化优势。 sceAPA 可以在单细胞水平检测各种癌症相关因素（编码基因、非编码区、假基因）相关的 APA 模式。在临床上，sceAPA 可广泛用于评估当前靶向治疗的隐藏反应者、新颖的个体化治疗策略以及组织和液体活检领域的药物组合。
